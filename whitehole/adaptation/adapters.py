...
class AdapterObjectiveConfig(ConfigBase):
    # Align adapted transitions with source-model transitions.
    alignment_weight: float = 1.0

    # Match multi-step predictions with adapted source latents.
    multistep_weight: float = 1.0
    multistep_discount: float = 1.0

    # Preserve local latent geometry; this is zero for ConstantOffset.
...
def dynamics_alignment_loss(
    ...
    """One-step transition alignment loss.

    L_align = || P(A(z_t), a_t) - A(z_{t+1}) ||^2
    """
...
def multistep_rollout_loss(
    ...
    """Multi-step rollout loss, starting at h=2."""
