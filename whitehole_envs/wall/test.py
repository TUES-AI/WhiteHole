...
from whitehole_envs.wall.data.offline_wall import (
    OfflineWallDataset,
...
def test_offline_ds(path):
    ds_wall = OfflineWallDataset(
        OfflineWallDatasetConfig(
            offline_data_path=str(path),
        )
    )
    sample = ds_wall[0]
...
def main():
    ...
    offline_data_path = Path("presaved_datasets/good_quality_data.npz")
    if offline_data_path.exists():
        test_offline_ds(offline_data_path)
