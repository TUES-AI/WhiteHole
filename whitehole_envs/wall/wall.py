...
from whitehole_envs.wall.data.wall_utils import check_wall_intersect
...
class DotWall(gym.Env):
    metadata = {"render_modes": ["observation"], "render_fps": 10}
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=255, shape=(2, img_size, img_size), dtype=np.uint8
        )
    ...
    def reset(
        ...
        *,
        ...
        if location is not None:
            self.dot_position = self._as_position_tensor(location)
        if target_location is not None:
            self.target_position = self._as_position_tensor(target_location)
    ...
    def _as_position_tensor(self, location) -> torch.Tensor:
        return torch.as_tensor(location, device=self.device, dtype=torch.float32)
    ...
    def step(self, action: np.array) -> Tuple[ObsType, float, bool, bool, InfoType]:
        ...
        if action_norm > self.max_step_norm:
            action = action * (self.max_step_norm / action_norm.clamp_min(1e-8))

        self.dot_position = self._calculate_next_position(action)
        self.position_history.append(self.dot_position)
        obs = self._render_dot_and_wall()
        done = bool(
            ((self.dot_position - self.target_position).pow(2).mean() < 1.0).item()
        )
        truncated = len(self.position_history) >= self.n_steps
        reward = 1.0 if done else 0.0
        return obs, reward, done, truncated, self._build_info()
