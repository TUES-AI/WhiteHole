...
- **Adaptation placement is a first-class variable:** MoVie adapts RGB and first-convolution features, not a late latent alone. Its 0–4 STN ablation only adds spatial warps progressively through the CNN and never tests a parameter-matched post-encoder latent adapter, so it does not establish that shallow adaptation is better than post-encoder adaptation. WhiteHole already has both a small input-space Reacher adapter ([`SmallConvAdapter`](../../../scripts/reacher_conv_adapter.py)) and post-encoder Two-Room latent adapters, enabling the missing matched comparison for geometric versus appearance shifts.
